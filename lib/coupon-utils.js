...
async function getCouponsMap(configOverride = null) {
  const hasConfigOverride = Boolean(configOverride);
  const config = hasConfigOverride ? configOverride : await getStoreConfig();
  ...
  Object.entries(merged).forEach(([code, coupon]) => {
    if (!coupon || typeof coupon !== "object") return;
    merged[code] = {
      ...coupon,
      code: coupon.code || code,
      hidden: Boolean(coupon.hidden),
      allowWithPackage: coupon.allowWithPackage !== false
    };
  });
  ...
  if (config.couponRuntime.hideHomeCouponsAfterExpiry || config.couponRuntime.lastAutoExpiredAt) {
    config.couponRuntime.hideHomeCouponsAfterExpiry = false;
    config.couponRuntime.lastAutoExpiredAt = "";
    changed = true;
  }

  if (changed && !hasConfigOverride) {
...
async function getCouponByCode(code, configOverride = null) {
  const normalizedCode = normalizeCouponCode(code);
  if (!normalizedCode) return null;
  const coupons = await getCouponsMap(configOverride);
...
async function validateCoupon(code, context = {}) {
  ...
  const coupon = context.couponsMap?.[normalizedCode] || await getCouponByCode(normalizedCode, context.storeConfig || null);
  if (!coupon) {
  ...
  const hasPackage = Boolean(context.hasPackage);

  if (hasPackage && coupon.allowWithPackage === false) {
    return {
      valid: false,
      error: "Este cupom ṇo combina com pacote promocional."
    };
  }
...
async function registerCouponUsage(code, paymentId) {
  ...
  const coupons = await getCouponsMap(config);
